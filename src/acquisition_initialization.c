...
#define FAST_REACQUISITION_CHANNEL_NUMBER   9
    ...
    switch( ctype ){
        ...
        case CTYPE_E1B:
            acq->noncoherentintegration[0]=ACQINTG_E1B;
            acq->noncoherentintegration[1]=ACQINTG_E1B;
            acq->noncoherentintegration[2]=ACQINTG_E1B;
            break;

        case CTYPE_B1I:
            acq->noncoherentintegration[0]=ACQINTG_B1I;
            acq->noncoherentintegration[1]=ACQINTG_B1I;
            acq->noncoherentintegration[2]=ACQINTG_B1I;
            break;
        ...
        case CTYPE_L2CM:
            acq->noncoherentintegration[0] = ACQINTG_L2CM;
            acq->noncoherentintegration[1] = ACQINTG_L2CM;
            acq->noncoherentintegration[2] = ACQINTG_L2CM;
            break;
    ...
    switch( _numcoherentcodes_ ){
        ...
        case 2:
            acq->hband  = 5000;
            acq->step   = 250;
            acq->nfreq  = 2*acq->hband/acq->step + 1;

            acq->numfreq[0]     = 41;
            acq->numfreq[1]     = FAST_REACQUISITION_CHANNEL_NUMBER;
            acq->numfreq[3]     = FAST_REACQUISITION_CHANNEL_NUMBER;
            break;
    ...
    if( !( acq->freq_4thHz[0] = (int16_t*) malloc( sizeof(int16_t) * acq->numfreq[0] ) ) ) {
        debug_print("memory allocation error: acq->freq0_4thHz. halfband_Hz=%d, step_128thHz=%d, numfreq=%d\n", 
                acq->halfband_Hz, acq->step_128thHz, acq->numfreq[0] ); 
        return -1;
    }
    for( ii = 0; ii < acq->numfreq[0]; ii++ ){
        acq->freq_4thHz[0][ii] = ( intfreq_Hz + offset_Hz ) * 4 + ( ii - acq->numfreq[0]/2.0 + 0.5 ) * ( acq->step_128thHz >> 5 ) ;                            
    }
...
//if( 1 ){ int16_t * y=(int16_t*) acq->freq_4thHz[0]; printf( "\nnumfreq[0]=%d freq_4thHz[0]=", acq->numfreq[0]); for(int zz=0; zz<acq->numfreq[0]; zz+=2) printf(" %d ", y[zz] ); printf("\n"); }
    ...
    for( ii = 0; ii < acq->numfreq[1]; ii++ ){
        acq->freq_4thHz[1][ii] = ( ii - acq->numfreq[1]/2.0 + 0.5 ) * ( acq->step_128thHz >> 5 );
    }
...
//debug_print("coherencelen_code=[%d %d] acq->fftlen=[%d %d]\n", coherencelen_code[0], coherencelen_code[1], acq->fftlen[0], acq->fftlen[1] );
    return 0;
                        ...
                        sdrch_t *sdr )
{
    ...
    sdr->nsamp = (int)(f_sf*sdr->ctime);  const unsigned int nsamp = sdr->nsamp ;
    ...
    uint16_t coherencelen_code[2];
    uint32_t coherencelen_sample[2];
    switch( _numcoherentcodes_ ){
        case 15:
            coherencelen_code[0] = 15;
            coherencelen_code[1] = 15;
            coherencelen_sample[0] = nsamp * coherencelen_code[0];
            coherencelen_sample[1] = nsamp * coherencelen_code[1];
            break;
        default:
            coherencelen_code[0] = _numcoherentcodes_;
            coherencelen_code[1] = 15;
            coherencelen_sample[0] = nsamp * coherencelen_code[0];
            coherencelen_sample[1] = nsamp * coherencelen_code[1];
    }
    ...
    sdr->numcoherentsamples = nsamp*sdr->numcoherentcodes ;

    if( initialize_acqusition_struct( 
                &sdr->acq, 
                ctype,
                coherencelen_code,
                nsamp, 
...
debug_print("fftlen_bit=[%d %d] fftlen=[%d %d] numcoherentsamples=%d numcoherentcodes=%d nfreq=[%d %d] sdr->ci=%f\n", 
        ...
        sdr->numcoherentsamples, coherencelen_sample[0], sdr->acq.numfreq[0], sdr->acq.numfreq[1], sdr->ci ); 
**/

    unsigned int fftlength = sdr->acq.fftlen[0];
    float scalefactor = 1.0 / ( 1 )  ;
    ...
    switch( coherencelen_code[0] ){
        case 15:
            ...
            break;
        default:
            zeropadding_sample = fftlength - coherencelen_sample[0] ;
    }
    ...
    for( t=0; ii < fftlength-sdr->nsamp; ii++, t += sdr->ci ) rcode[ii] = sdr->code[ ( (int) t) % sdr->clen ] * scalefactor ;
    for(    ; ii < (fftlength + zeropadding_sample ); ii++ ) rcode[ii] = 0.0 ;


//if( 1 ){ float *y = (float *) rcode; printf("\n%d rcode=", sdr->no); for(int x=zeropadding_sample; x<zeropadding_sample+80; x++) printf( " %e", y[x]  ); printf("\n"); }

#if defined(FFTW_ENABLED) || defined(FFTWF_ENABLED) 
...
//if( sdr->no > 0 ){ float *y = (float*) rcode; printf( "zeropadding_sample=%d rcode = [", zeropadding_sample ); for(int zz=0; zz<20; zz++) printf(" %.2e ", y[zz] ); printf("]\n"); }
...
//if( sdr->no > 0 ){ float * y = (float*) sdr->acq.xcode[0]; printf( "%d 1 sdr->acq.xcode[0] = [", sdr->no ); for(int zz=0; zz<10; zz++) printf(" %.1e ", y[zz] ); printf("]\n"); }
...
#if defined( FFTWF_ENABLED)
    fftwf_plan_with_nthreads(NFFTTHREAD);
#endif
    plan_f = fftwf_plan_dft_1d( fftlength, sdr->acq.xcode[0], sdr->acq.xcode[0], FFTW_FORWARD, FFTW_ESTIMATE );   /**Once the plan has been created, you can use it as many times as you like for transforms on arrays of the same size.**/
    ...
    //fftwf_destroy_plan( plan_f ); plan_f = fftwf_plan_dft_1d( fftlength, sdr->acq.xcode[1], sdr->acq.xcode[1], FFTW_FORWARD, FFTW_ESTIMATE);
...
//if( sdr->no > 0 ){ float * y = (float*) sdr->acq.xcode[0]; printf( "%d 2 sdr->acq.xcode[0] = [", sdr->no ); for(int zz=0; zz<10; zz++) printf(" %.2e ", y[zz] ); printf("]\n"); }
...
//if( sdr->no > 0 ){ double * y = (double*) sdr->xcode; printf( "\n%s fftlength=%d xcode = [", sdr->satstr, fftlength); for(int zz=0; zz<20; zz++) printf(" %.3e ", y[zz] ); printf("]\n"); }
    ...
    //fftw_destroy_plan( plan_d ); plan_d = fftw_plan_dft_1d( fftlength, sdr->codex, sdr->codex, FFTW_FORWARD, FFTW_ESTIMATE);
...
//if( sdr->no > 0 ){ float * y = (float*) sdr->acq.xcode[0]; printf( "%s xcode[0] = [", sdr->satstr ); for(int zz=0; zz<20; zz++) printf(" %.1e ", y[zz] ); printf("]\n"); }
//if( sdr->no > 0 ){ double * y = (double*) sdr->xcode; printf( "\n%s fftlength=%d xcode = [", sdr->satstr, fftlength); for(int zz=0; zz<20; zz++) printf(" %.3e ", y[zz] ); printf("]\n"); }
...
//debug_print("no=%2d d[0:1] = [%.3f %.3f] p[0:1] = [%.3f %f] pp[0:1]=[%.3f %.3f] q[0:1]=[%.3f %.3f] qq[0:1]=[%.3f %.3f]\n", sdr->no, d[0], d[1], p[0], p[1], pp[0], pp[1], q[0], q[1], qq[0], qq[1] );
    ...
    for( t=0; i<fftlength-sdr->nsamp; i++, t += sdr->ci ) rcode[i] = sdr->code[ ((int) t) % sdr->clen ] * scalefactor ;
